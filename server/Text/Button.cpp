...
Button::Button(const char* _text, TTF_Font *_font, int _x, int _y, int _thickOfBorder, SDL_Color _textColor, bool _isVisible) {
	text = _text;
	font = _font;
	border.x = _x;
	border.y = _y;
...
void Button::TextUpdate(const char *upd) {
	text = upd;
}
